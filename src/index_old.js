...
function initializeScrollytelling() {
    ...
    function goToSection(sectionIndex) {
        ...
        sectionsContainer.scrollTo({
            top: targetSection.offsetTop - 80, // -80px per l'header fisso
            behavior: 'smooth'
        });
...
// Wheel scrolling ma con con debouncing sofisticato per evitare che scrolli più di una view alla volta
let lastWheelTime = 0;
let wheelTimeout;
...
sectionsContainer.addEventListener('wheel', (e) => {
    ...
    // Blocca immediatamente ulteriori eventi wheel
    isWheelBlocked = true;
    
    // Clear del timeout precedente
    clearTimeout(wheelTimeout);
    
    // Debounce: esegui l'azione solo dopo 50ms di "silenzio"
    wheelTimeout = setTimeout(() => {
        ...
        // Rilascia il blocco dopo l'animazione completa (600ms = durata goToSection)
        setTimeout(() => {
...
// Alternativa ancora più semplice e reattiva:
sectionsContainer.addEventListener('wheel', (e) => {
    e.preventDefault();
    ...
    // Throttling più leggero - solo 250ms
    if (now - lastWheelTime < 250) return;
    
    // Soglia minima per evitare scroll accidentali
    ...
    // Detecta la sezione attuale durante lo scroll manuale (SEMPLIFICATA)
    ...
    // Gestione del tocco per dispositivi mobili (MIGLIORATA)
    let touchStartY = 0;
    let touchEndY = 0;
