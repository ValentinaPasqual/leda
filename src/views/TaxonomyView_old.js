...
export class TaxonomyView {
  ...
  buildHierarchy(data, indexKey) {
    ...
    data.forEach(item => {
      ...
      rawValues.forEach(val => {
        ...
        // Aggiungi l'item SOLO al nodo finale (foglia)
  ...
  goToMapWithFilter(taxonomyValue) {
    ...
    const mapUrl = window.location.origin + base + 'pages/mappa.html' + `?${params.toString()}`;
...
convertHierarchyToD3Format(obj, name = "root") {
  ...
  renderSunburst() {
    ...
    // Aggiungi questo subito dopo la creazione del root
    ...
    const arc = d3.arc()
    .startAngle(d => d.x0 * angleScale)  // ← Normalizzato
    .endAngle(d => d.x1 * angleScale)    // ← Normalizzato
      .innerRadius(d => Math.max(0, d.y0))
      .outerRadius(d => d.y1);

    console.log(`Normalizzazione: ${maxAngleUsed.toFixed(3)} → ${(2 * Math.PI).toFixed(3)} (scala: ${angleScale.toFixed(3)})`);

    // Verifica anche alcuni angoli prima e dopo la normalizzazione
    root.descendants().filter(d => d.depth === 1).forEach(d => {
      console.log(`${d.data.name}: ${d.x0.toFixed(3)}-${d.x1.toFixed(3)} → ${(d.x0 * angleScale).toFixed(3)}-${(d.x1 * angleScale).toFixed(3)}`);
    });
    ...
    svg.selectAll("path")
      .data(root.descendants()) // .data(root.descendants().filter(d => d.depth)) --> per filtrare la root dalla visualizzazione
      .enter()
      .append("path")
      .attr("d", arc)
